...
class parameters:
    def __init__(self):
        # px and py are the coordinates of the point P in the robot frame
        # The point P is the point on the robot that should be on the path
        self.px = 0.05
        self.py = 0
        self.Kp = 100;
        self.R = 0.1
...
x0 = c[0]
y0 = c[1]
current_state = np.array([x0, y0, theta0]); #x, y, theta at the start for point C
...
for i in range(0,len(t)-1):
    ...
    p0 = ptC_to_ptP(current_state[0],current_state[1],current_state[2],parms);
    x_p = p0[0]
    y_p = p0[1]
    ...
    control_input = np.matmul(Ainv,np.transpose(b))
    #u = Ainv
    #*b; #%u = [v omega]
    v.append(control_input[0])
    omega.append(control_input[1])

    #     % 5. now control the car based on u = [v omega]
    current_state = euler_integration([t[i], t[i+1]],current_state,[control_input[0],control_input[1]])
...
f_z1 = interpolate.interp1d(t, states[:,0])
f_z2 = interpolate.interp1d(t, states[:,1])
f_z3 = interpolate.interp1d(t, states[:,2])
